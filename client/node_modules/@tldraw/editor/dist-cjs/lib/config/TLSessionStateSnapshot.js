...
const TAB_ID = window ? window[tabIdKey] ?? (0, import_utils.getFromSessionStorage)(tabIdKey) ?? `TLDRAW_INSTANCE_STATE_V1_` + (0, import_uniqueId.uniqueId)() : "<error>";
if (window) {
...
function loadSessionStateSnapshotIntoStore(store, snapshot) {
  ...
  const instanceState = store.schema.types.instance.create({
    ...
    ...(0, import_tlschema.pluckPreservingValues)(store.get(import_tlschema.TLINSTANCE_ID)),
