...
class Editor extends import_eventemitter3.default {
  constructor({
    ...
    this.timers = new import_utils.Timers();
    ...
    this._assetOptions.set({ ...import_constants.DEFAULT_ASSET_OPTIONS, ...assetOptions });
  ...
  setHintingShapes(shapes) {
    const ids = typeof shapes[0] === "string" ? shapes : shapes.map((shape) => shape.id);
    this.updateCurrentPageState({ hintingShapeIds: (0, import_utils.dedupe)(ids) }, { history: "ignore" });
    return this;
  }
  getErasingShapeIds() {
          ...
          switch (behaviorX) {
            ...
            case "contain": {
              if (z < zx)
                ...
                x = (0, import_utils2.clamp)(x, minX + freeW, minX);
              break;
            }
            case "inside": {
              if (z < zx)
                x = (0, import_utils2.clamp)(x, minX, (vsb.w - px) / z - bounds.w);
              else
                x = (0, import_utils2.clamp)(x, minX + freeW, minX);
              break;
            }
          ...
          switch (behaviorY) {
            ...
            case "contain": {
              if (z < zy)
                ...
                y = (0, import_utils2.clamp)(y, minY + freeH, minY);
              break;
            }
            case "inside": {
              if (z < zy)
                y = (0, import_utils2.clamp)(y, minY, (vsb.h - py) / z - bounds.h);
              else
                y = (0, import_utils2.clamp)(y, minY + freeH, minY);
              break;
            }
  ...
  /* --------------------- Assets --------------------- */
  _assetOptions = (0, import_state.atom)("asset options", import_constants.DEFAULT_ASSET_OPTIONS);
  _getAllAssetsQuery() {
  ...
  _notVisibleShapes() {
    return (0, import_notVisibleShapes.notVisibleShapes)(this);
  }
  getCulledShapes() {
      ...
      const { shapesToCreate, bindingsToCreate } = withIsolatedShapes(
        ...
        (bindingIdsToMaintain) => {
          ...
          for (const originalId of orderedShapeIds) {
            ...
            const index = siblingAbove ? (0, import_utils.getIndexBetween)(originalShape.index, siblingAbove.index) : (0, import_utils.getIndexAbove)(originalShape.index);
  ...
  groupShapes(shapes, options = {}) {
    const { groupId = (0, import_tlschema.createShapeId)(), select = true } = options;
    if (!Array.isArray(shapes)) {
  ...
  ungroupShapes(shapes, options = {}) {
    ...
    const groups = [];
    (0, import_utils.compact)(ids.map((id) => this.getShape(id))).forEach((shape) => {
      if (this.isShapeOfType(shape, "group")) {
        groups.push(shape);
      } else {
  ...
  async getSvgElement(shapes, opts = {}) {
    const result = await (0, import_getSvgJsx.getSvgJsx)(this, shapes, opts);
    if (!result)
      return void 0;
  ...
  loadSnapshot(snapshot) {
    (0, import_TLEditorSnapshot.loadSnapshot)(this.store, snapshot);
    return this;
  }
  /**
      ...
      case "pointer": {
        ...
        switch (info.name) {
          case "pointer_down": {
            ...
            if (info.button === import_constants.LEFT_MOUSE_BUTTON)
