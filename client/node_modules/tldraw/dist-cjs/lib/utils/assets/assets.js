...
async function downsizeImage(blob, width, height, opts = {}) {
  const image = await import_editor.MediaHelpers.usingObjectURL(blob, import_editor.MediaHelpers.loadImage);
  const { type = blob.type, quality = 0.92 } = opts;
  const [desiredWidth, desiredHeight] = await (0, import_getBrowserCanvasMaxSize.clampToBrowserMaxCanvasSize)(
