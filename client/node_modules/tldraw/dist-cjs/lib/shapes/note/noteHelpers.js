...
__export(noteHelpers_exports, {
  ADJACENT_NOTE_MARGIN: () => ADJACENT_NOTE_MARGIN,
...
const BASE_NOTE_POSITIONS = [
  [["a1"], new import_editor.Vec(NOTE_SIZE * 0.5, NOTE_SIZE * -0.5 - ADJACENT_NOTE_MARGIN)],
  // t
  [["a2"], new import_editor.Vec(NOTE_SIZE * 1.5 + ADJACENT_NOTE_MARGIN, NOTE_SIZE * 0.5)],
  // r
  [["a3"], new import_editor.Vec(NOTE_SIZE * 0.5, NOTE_SIZE * 1.5 + ADJACENT_NOTE_MARGIN)],
  // b
  [["a4"], new import_editor.Vec(NOTE_SIZE * -0.5 - ADJACENT_NOTE_MARGIN, NOTE_SIZE * 0.5)]
  // l
];
...
function getAvailableNoteAdjacentPositions(editor, rotation, scale, extraHeight) {
  ...
  for (const [shape, center] of allCenters) {
    for (let i = 0; i < len; i++) {
      ...
      if (!position)
        continue;
      if (import_editor.Vec.Dist2(center, position) > minSize)
        continue;
...
function getNoteShapeForAdjacentPosition(editor, shape, center, pageRotation, forceNew = false) {
  ...
  (0, import_selectHelpers.zoomToShapeIfOffscreen)(editor);
  return nextNote;
}
