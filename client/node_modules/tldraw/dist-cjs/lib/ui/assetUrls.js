...
let defaultUiAssetUrls = {
  ...
  icons: Object.fromEntries(
    import_icon_types.iconTypes.map((name) => [name, `${(0, import_assetUrls.getDefaultCdnBaseUrl)()}/icons/icon/${name}.svg`])
  ),
  translations: Object.fromEntries(
    import_editor.LANGUAGES.map((lang) => [
      lang.locale,
      `${(0, import_assetUrls.getDefaultCdnBaseUrl)()}/translations/${lang.locale}.json`
    ])
  ),
  embedIcons: Object.fromEntries(
    import_editor.EMBED_DEFINITIONS.map((def) => [
      def.type,
      `${(0, import_assetUrls.getDefaultCdnBaseUrl)()}/embed-icons/${def.type}.png`
    ])
  )
};
